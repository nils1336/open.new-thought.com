...
/**
 * send-result.php
 * Sendet E-Mail-Benachrichtigung an Nils wenn jemand den KI-Readiness-Check abschließt.
 */
...
// ── Email body ────────────────────────────────────────────────
$date      = date('d.m.Y, H:i');
$replyLink = "mailto:{$visitorEmail}?subject=" . rawurlencode('Ihr KI-Readiness-Ergebnis - Gespraech anbieten')
...
<div style="max-width:580px;margin:0 auto;background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.09);">
  ...
  <div style="padding:28px 36px 0;">
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:26px;">
      <tr>
        <td style="width:50%;padding-right:12px;vertical-align:top;">
          <div style="font-size:11px;letter-spacing:.15em;text-transform:uppercase;color:#aaa;margin-bottom:8px;font-family:monospace;">St&auml;rken</div>{$strongHtml}
        </td>
        <td style="width:50%;padding-left:12px;vertical-align:top;">
          <div style="font-size:11px;letter-spacing:.15em;text-transform:uppercase;color:#aaa;margin-bottom:8px;font-family:monospace;">Handlungsfelder</div>{$weakHtml}
        </td>
      </tr>
    </table>
    <div style="margin-bottom:26px;">
    ...
    <div style="margin-bottom:36px;">
      <a href="{$replyLink}" style="display:inline-block;background:#ff3eb5;color:#fff;padding:14px 26px;border-radius:8px;text-decoration:none;font-size:14px;font-weight:700;">Direkt antworten</a>
    </div>
  </div>
...
// ── Send ──────────────────────────────────────────────────────
$subject = '=?UTF-8?B?' . base64_encode('KI-Check: ' . $visitorEmail . ' | ' . $overall . '/10 | ' . $profileType) . '?=';
$headers = implode("\r\n", [
    'MIME-Version: 1.0',
    'Content-Type: text/html; charset=UTF-8',
    'From: KI-Check new.thought <nils@new-thought.com>',
...
// ── Log ───────────────────────────────────────────────────────
$logLine = date('Y-m-d H:i:s') . "\t" . ($sent ? 'SENT' : 'FAILED') . "\t{$visitorEmail}\t{$overall}/10\t{$profileType}\n";
